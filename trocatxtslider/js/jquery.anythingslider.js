...
;(function($, win, doc) {
	...
	$.anythingSlider = function(el, options) {
		...
		base.gotoPage = function(page, autoplay, callback, time) {
			...
			// Quando a reprodução automática não é passada, paramos o cronômetro
			if (autoplay !== true) { autoplay = false; }
			// Pare o controle deslizante quando chegarmos à última página, se a opção stopAtEnd estiver definida como true
			...
			if (playing){
				...
				base.timer = win.setInterval(function() {
					if ( !!(doc.hidden || doc.webkitHidden || doc.mozHidden || doc.msHidden) ) {
						...
						// impedir a reprodução automática se o vídeo estiver sendo reproduzido
						base.goForward(true, o.playRtl);
...
*/
		...
		// Slideshow options
		autoPlay            : true,     // Se for verdade, a apresentação de slides começará a ser executada; substitui a opção "startStopped"
		autoPlayLocked      : false,     // Se for verdade, o usuário alterando os slides não interromperá a apresentação de slides
		autoPlayDelayed     : false,     // Se for verdade, iniciar uma apresentação de slides atrasará o avanço dos slides; se falso, o controle deslizante avançará imediatamente para o próximo slide quando a apresentação de slides começar
		pauseOnHover        : true,      // If true & the slideshow is active, the slideshow will pause on hover
		stopAtEnd           : false,     // If true & the slideshow is active, the slideshow will stop on the last page. This also stops the rewind effect when infiniteSlides is false.
		playRtl             : false,     // If true, the slideshow will move right-to-left

		// Times
		delay               : 9000,      // o movimento Quanto tempo entre as transições da apresentação de slides no modo Reprodução Automática (em milissegundos)
		resumeDelay         : 15000,   // Retomar a apresentação de slides após a interação do usuário, somente se autoplayLocked for true (em milissegundos).
		animationTime       : 600,       // 600 orig Quanto tempo dura a transição da apresentação de slides (em milissegundos)
		delayBeforeAnimate  : 0,      // Quanto tempo pausar a animação do slide antes de ir para o slide desejado (usado se você quiser que seu "out" FX seja exibido).
